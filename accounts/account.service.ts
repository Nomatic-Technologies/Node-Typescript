...
async function authenticate({ email, password, ipAddress }:{ email:any, password:any, ipAddress:any }) {
...
async function refreshToken({ token, ipAddress }:{ token:any, ipAddress:any }) {
...
async function revokeToken({ token, ipAddress }:{ token:any, ipAddress:any }) {
    const refreshToken = await getRefreshToken(token);
...
async function verifyEmail({ token }:{token:any}) {
...
async function forgotPassword({ email }:{email:any}, origin:any) {
    const account = await db.Account.findOne({ email });
...
async function validateResetToken({ token }:{token:any}) {
...
async function resetPassword({ token, password }:{token:any,password:any}) {
    const account = await db.Account.findOne({
        'resetToken.token': token,
        'resetToken.expires': { $gt: Date.now() }
    });
